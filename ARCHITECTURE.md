...
#### 8.6.4 决策链显示与“生效参数”面板（扫描/回测/调试器）

本功能用于把策略在某个交易日的逐步过滤/决策过程（`trace`）与当日关键环境值（如通道高度、斜率、量比等）直接展示出来，方便复盘与校验“为什么进/为什么不进”。

**扫描模块（点击行显示决策链）**

- 交互：点击扫描结果表格 `#scan-results` 的某一行，在下方展开 `#scan-trace-wrap` 并渲染该行对应的 `trace`（见 [app.js:scan trace](file:///f:/Projects/band-strategy/static_bak/app.js#L2089-L2156) 与 [index.html:scan-trace-wrap](file:///f:/Projects/band-strategy/static_bak/index.html#L800-L848)）。
- 数据来源：扫描结果的 `env/signals` 来自策略的 `signal_logs`；当 `capture_logs=True` 时会包含 `trace`，否则会显示“暂无决策链数据”。

**回测模块（点击成交行显示该笔交易的决策链）**

- 交互：点击“交易明细”中的某一成交行，在下方 `#bt-trace-wrap` 渲染该笔交易日期对应的决策链（见 [_btRenderTrace](file:///f:/Projects/band-strategy/static_bak/app.js#L4411-L4466) 与回测详情点击逻辑 [app.js](file:///f:/Projects/band-strategy/static_bak/app.js#L3970-L4065)）。
- 无成交兜底：当该标的无成交时，默认展示最后一个交易日的决策链（便于解释“为什么一直不买”）。
- 数据来源：`/api/backtest_detail` 的 `signal_logs[*].trace`（由 [EventBacktestResult.to_dict](file:///f:/Projects/band-strategy/core/event_engine.py#L222-L280) 返回），需要后端在明细回测时打开 `capture_logs`。

**侧栏“生效参数”面板（配置/运行/实际 三行）**

- 目的：区分“配置值”（预设/输入）、“运行值”（回测/调试时最终生效的配置）与“实际值”（交易日的通道/量比等环境指标）。
- 展示：统一由 [_renderParamsListPanel](file:///f:/Projects/band-strategy/static_bak/app.js#L4120-L4400) 渲染，按参数键展示三行：
  - `配置`：`__input_params`（或直接 params）
  - `运行`：`__running_params`（若与配置不同则展示）
  - `实际`：`__actual_env`（从 `signal_logs` 里抽取并做字段映射，如 `min_channel_height -> channel_height`）
