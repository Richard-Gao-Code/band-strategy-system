...
flowchart LR
  ...
  B --> H[core/data.py<br/>load_bars_from_csv/...]
  C --> H
...
入口：[api_scan](file:///F:/Projects/band-strategy/app.py#L610-L803)  
编排函数：[scan_channel_hf_for_symbol_path](file:///F:/Projects/band-strategy/core/scanner_runner.py#L363-L464)
...
- 请求体：`RunReq`（见 [app.py:L67-L121](file:///F:/Projects/band-strategy/app.py#L67-L121)）。
...
入口：
- 批量回测：[api_backtest](file:///F:/Projects/band-strategy/app.py#L828-L937)
- 单标的明细回测：[api_backtest_detail](file:///F:/Projects/band-strategy/app.py#L1062-L1117)

编排函数：[backtest_channel_hf_for_symbol_path](file:///F:/Projects/band-strategy/core/scanner_runner.py#L187-L361)
...
入口：[api_debug_analyze](file:///F:/Projects/band-strategy/app.py#L1640-L1687)  
编排函数：[debug_analyze_channel_hf](file:///F:/Projects/band-strategy/core/debug_runner.py#L428-L639)
...
引擎实现：[EventBacktestEngine.run](file:///F:/Projects/band-strategy/core/event_engine.py#L385-L558)
...
- URL：本项目为单页应用，入口统一为 `/`（见 [index()](file:///F:/Projects/band-strategy/app.py#L605-L607) 返回 [index.html](file:///F:/Projects/band-strategy/static_bak/index.html)），无独立 `/batch_test` 页面路由。
- 页面位置：位于“回测”视图（`view-backtest`）中的“批量参数测试”卡片。
- 关键按钮 ID：
  - `pt-run-btn`：开始/停止批量测试（见 [index.html](file:///F:/Projects/band-strategy/static_bak/index.html#L603-L633)）。
  - `pt-export-btn`：导出结果（CSV）（同上）。
- 前端主函数：`runParamBatchTest()`（见 [runParamBatchTest](file:///F:/Projects/band-strategy/static_bak/app.js#L1747)）。

**后端接口**

- `POST /api/param_batch_test`：批量参数测试（NDJSON 流式输出，逐行 JSON）（见 [api_param_batch_test](file:///F:/Projects/band-strategy/app.py#L939-L1056)）。
...
- 任务编排（组合循环 + 并发 + 流式输出）：[api_param_batch_test](file:///F:/Projects/band-strategy/app.py#L939-L1056)（在 API 层直接实现）。
- 单标的回测执行：`backtest_channel_hf_for_symbol_path(symbol, data_path, index_path, config)`（见 [scanner_runner.py](file:///F:/Projects/band-strategy/core/scanner_runner.py#L187-L361)）。
...
- 触发入口在“回测结果表格”的每行渲染逻辑：当该标的被拒绝（`error` 或 `reasons` 非空）时，渲染“查看详情”按钮：
  - `onclick="showRejectionDetails('${symbol}')"`（见 [renderBacktestRowToTbody](file:///F:/Projects/band-strategy/static_bak/app.js#L2622-L2671)）。
- 弹窗主函数：`showRejectionDetails(symbol)`（见 [showRejectionDetails](file:///F:/Projects/band-strategy/static_bak/app.js#L1544)）。

**后端 API（现状）**

- 后端提供两种等价接口：
  - `POST /api/backtest_detail`（见 [api_backtest_detail](file:///F:/Projects/band-strategy/app.py#L1062-L1117)）
  - `GET /api/backtest/detail`（见 [api_backtest_detail_get](file:///F:/Projects/band-strategy/app.py#L1137-L1171)），用于兼容前端 querystring 调用，内部转发到上述 POST。
- 前端拉取详情使用 `fetchBacktestDetailForSymbol(symbol)` 调 `GET /api/backtest/detail?...`（见 [fetchBacktestDetailForSymbol](file:///F:/Projects/band-strategy/static_bak/app.js#L2674-L2698)）。

**返回数据格式（后端当前实际结构）**

- `GET /api/backtest/detail` 与 `POST /api/backtest_detail` 最终返回结构一致：`EventBacktestResult.to_dict()`（见 [EventBacktestResult.to_dict](file:///F:/Projects/band-strategy/core/event_engine.py#L222-L280)）并额外补充 `symbol/beg/end`。
...
- 展示形态有两种：
  - 配置页内联说明：点击/聚焦配置项时，调用 `_renderParamHelpInline(key)` 写入 `#config-help-content`（见 [_renderParamHelpInline](file:///F:/Projects/band-strategy/static_bak/app.js#L1488-L1516) 与 [index.html:config-help-content](file:///F:/Projects/band-strategy/static_bak/index.html#L530)）。
  - 全量说明弹窗：`showParamHelpModal()` 遍历 `PARAM_DEFINITIONS` 动态生成弹窗内容（见 [showParamHelpModal](file:///F:/Projects/band-strategy/static_bak/app.js#L1968-L2053)）。
...
- 核心回测时序与触发条件存在一致性风险：引擎设计为“`T` 日收盘生成信号、`T+1` 开盘执行”（见 [EventBacktestEngine.run](file:///F:/Projects/band-strategy/core/event_engine.py#L385-L523)），但策略在 `on_close` 中会用当日 `bar.high/bar.low` 触发卖出/止损分支（如 `bar.high >= target_px`、`bar.low <= stop_px*(1-panic_eps)`，见 [ChannelHFStrategy.on_close 退出逻辑](file:///F:/Projects/band-strategy/core/channel_hf.py#L608-L669)）。这会造成“用日内极值触发，但在次日开盘成交”的口径混用，需要明确策略假设或改为更一致的撮合/触发方式。
- 扫描模式对 `ChannelHFConfig` 仅赋值了字段子集，部分策略参数即使在前端/预设中存在也不会生效（见 [scan_channel_hf_for_symbol_path 构造 ChannelHFConfig](file:///F:/Projects/band-strategy/core/scanner_runner.py#L363-L422)）。例如 Pivot 确认相关的 `pivot_confirm_days/pivot_no_new_low_tol/pivot_rebound_amp/...` 在 scan/backtest 编排中未统一透传。
- 配置项存在历史遗留字段：`RunReq` 与预设中包含 `slope_vol_max`，但当前主链路不使用（仅在备份文件里出现）（见 [RunReq:slope_vol_max](file:///F:/Projects/band-strategy/app.py#L96-L120) 与 [scanner_runner.py.bak](file:///F:/Projects/band-strategy/core/scanner_runner.py.bak)）。
- 参数默认值口径不一致，容易造成误解：`RunReq.scan_recent_days` 默认 `1`（见 [RunReq](file:///F:/Projects/band-strategy/app.py#L67-L121)），而 `ChannelHFConfig.scan_recent_days` 默认 `20`（见 [ChannelHFConfig](file:///F:/Projects/band-strategy/core/channel_hf.py#L38-L99)）；scan 编排里又会把 `config.get("scan_recent_days", 1)` 作为阈值（见 [scan_channel_hf_for_symbol_path:recent_n](file:///F:/Projects/band-strategy/core/scanner_runner.py#L427-L437)）。三处默认值并非同一口径。
